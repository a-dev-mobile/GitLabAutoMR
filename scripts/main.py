...
def parse_arguments():
    ...
    parser.add_argument("--is_draft", action='store_true', help="Создать MR как draft.")
    parser.add_argument("--mr_title", required=True, help="Заголовок Merge Request.")
    parser.add_argument("--branch_desc", required=True, help="Дополнительный текст к названию ветки.")
    parser.add_argument("--branch_type", default="feature", help="Тип ветки (feature, hotfix, bugfix, release).")
...
def main():
    try:
        ...
        print("\n--- Предварительный просмотр данных MR ---\n")
        
        mr_title_display = f"{'Draft: ' if args.is_draft else ''}{mr_title}"
        mr_info = [
            ("Project", project_name),
            ("Title", mr_title_display),
